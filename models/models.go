...
func CheckUserExists(tx *sqlx.Tx, userID string) error {
	...
	if err != nil {
		return fmt.Errorf("ユーザーの存在チェックに失敗しました: %w", err)
	}
	if count == 0 {
...
func CheckDuplicateTransaction(tx *sqlx.Tx, transactionID string) error {
	...
	if err != nil {
		return fmt.Errorf("重複リクエストのチェックに失敗しました: %w", err)
	}
	if count > 0 {
...
func UpdateBalance(tx *sqlx.Tx, userID string, amount int, effectiveDate time.Time) error {
    ...
    `, userID)
	if err != nil {
		return fmt.Errorf("現在の残高の取得に失敗しました: %v", err)
	}
	...
	return func(next echo.HandlerFunc) echo.HandlerFunc {
		return func(c echo.Context) error {
			...
			if err := next(c); err != nil {
				tx.Rollback()
				return fmt.Errorf("取引処理が失敗しました: %w", err)
			}
